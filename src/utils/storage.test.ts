...
describe('storage utilities (local-first)', () => {
  it('sorts stops putting favorites first then alphabetically', () => {
    const mixed: Stop[] = [
      { id: 'b', name: 'Beta', isFavorite: false, code: 'B', area: 'Test', lines: [] },
      { id: 'a', name: 'Alpha', isFavorite: true, code: 'A', area: 'Test', lines: [] },
      { id: 'c', name: 'Charlie', isFavorite: false, code: 'C', area: 'Test', lines: [] },
    ]
  ...
  it('restores active line only when valid for the selected stop', () => {
    localStorage.setItem(
      'public-transport-tracker.active-line',
      '18',
    )
  ...
  it('reads saved recent filter/sort preferences with safe fallback', () => {
    ...
    localStorage.setItem(
      'public-transport-tracker.recent-stop-sort',
      'urgent',
    )
