...
export function useTransportTrackerState() {
  ...
  const [recentStopViews, setRecentStopViews] = useState(() => getInitialRecentStopViews());
  ...
  const recentStops = recentStopViews
    ...
    .filter((entry): entry is (typeof recentStopViews)[number] & { stop: Stop } => Boolean(entry));
  ...
  useEffect(() => {
    window.localStorage.setItem(storageKeys.recentStops, JSON.stringify(recentStopViews));
  }, [recentStopViews]);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(storageKeys.selectedArrivals, JSON.stringify(remainingSelectedArrivals));
  ...
  const handleFavoriteToggle = (stopId: string) => {
    setStops((currentStops) =>
      sortStops(currentStops.map((stop) => (stop.id === stopId ? { ...stop, isFavorite: !stop.isFavorite } : stop))),
    );
  };
